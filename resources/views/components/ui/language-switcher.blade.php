...
<a href="{{ localize($code) }}"@class(['border-2 px-2 py-0.5', $isCurrent ? 'border-black bg-black text-white' : 'border-black hover:bg-black hover:text-white']) hreflang="{{ $code }}" lang="{{ $code }}" dir="{{ $code === 'ar' ? 'rtl' : 'ltr' }}" aria-current="{{ $isCurrent ? 'true' : 'false' }}">{{ $label }}</a>
@endforeach
</div>
@endif