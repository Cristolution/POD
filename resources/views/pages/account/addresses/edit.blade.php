@extends('layouts.app', ['title' => __('addresses_edit_title')])
...
@section('content')
    <section class="max-w-6xl mx-auto px-6 py-12">
        <x-layout.breadcrumbs :items="[
            ...
            '#' . $address->id => route('account.addresses.edit', $address),
        ]" />

        <h1 class="heading-1 mb-8">{{ __('addresses_edit_heading') }}<span class="text-coral-500">.</span></h1>

        <div class="grid md:grid-cols-[240px_1fr] gap-8">
            <x-layout.account-sidebar />

            <form method="POST" action="{{ route('account.addresses.update', $address) }}" class="card space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="btn-coral">{{ __('addresses_update') }}</button>
